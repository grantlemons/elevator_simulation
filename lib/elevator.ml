...
let rec run_simulation ?(delayed_calls=[]) queue elevators top_floor =
  ...
  match event with
    | Board(_, elevator, floor) -> run_simulation (PriorityQueue.insert rem_queue (distance_from_destination elevator.direction elevator.floor floor) event) elevators top_floor ~delayed_calls:delayed_calls
    | Exit(_, elevator, floor) -> run_simulation (PriorityQueue.insert rem_queue (distance_from_destination elevator.direction elevator.floor floor) event) elevators top_floor ~delayed_calls:delayed_calls
    | _ -> ()
  in
  ...
  let board_handler person elevator floor = if elevator.current_headcount < elevator.capacity
    then
      begin
        ...
        person.board_time <- Some (max_elapsed elevators);
  ...
  let exit_handler person elevator floor =
    ...
    person.exit_time <- Some (max_elapsed elevators);
  ...
  let call_handler person floor = 
    let is_valid_call elevator floor = match (elevator.direction, direction floor person.destination) with
      ...
      | _ -> false
    in
    person.call_time <- Some (max_elapsed elevators);
  ...
  let toggle_handler () =
    ...
    run_simulation (List.fold_left (fun q call -> PriorityQueue.insert q min_int call) PriorityQueue.Empty delayed_calls) elevators top_floor
  in
